...
function send_ajax_and_set_errors_exceptions_success(type){
	...
	if(type=="add"){
		...
		formData["zone_name"]=$("#zone_name").val();
	...
	$.ajax({
		...
		success: function (data, textStatus, jqXHR) {
			...
			if(!(data.zone===null) && data.zone.hasOwnProperty('type')){
				
				msg+="Zone: <strong>"+data.zone.type+"</strong> successfully "+operated+".";
			}
			else{
				//delete case
				if(!(data.zone===null) && data.zone>=1){
					msg+="Zone: <strong>"+formData.type+"</strong> successfully "+operated+".";
				}
				else{
