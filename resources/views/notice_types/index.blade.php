...
--}}
@extends('layouts.app') @section('title', 'Notice Types')
@section('page_heading') Notice Types @endsection
...
<div id="info-panel" class="panel panel-default">
	...
	<div class="panel-body">
		...
		<form id="info-form" class="form-horizontal" role="form" method="POST"
			action="{{ url('/admin/Notice-Type') }}">
			{{ csrf_field() }}
			<input type="hidden" id="notice_type-id">
			<div id="type-group" class="form-group our-form-group">
				<!-- IIIIIIIIIII -->
				<label for="type" class="col-md-4 control-label">Notice Type</label>

				<div class="col-md-6">
					<input id="type" type="text"
						class="form-control info-form-control" name="type"> <span
						id="type-span" class="help-block our-help-block"> <!-- IIIIIIIIIII -->
						<strong id="type-strong" class="our-error-message-strong"></strong>
						<!-- IIIIIIIIIII -->
					</span>
				</div>
			</div>

			<div id="description-group" class="form-group our-form-group">
				<!-- IIIIIIIIIII -->
				<label for="description" class="col-md-4 control-label">Description</label>

				<div class="col-md-6">
					<textarea id="description" rows="3"
						class="form-control info-form-control" name="description"></textarea> <span
						id="description-span" class="help-block our-help-block"> <!-- IIIIIIIIIII -->
						<strong id="description-strong" class="our-error-message-strong"></strong>
						<!-- IIIIIIIIIII -->
					</span>
				</div>
			</div>

			<div id="file_prefix-group" class="form-group our-form-group">
				<!-- IIIIIIIIIII -->
				<label for="file_prefix" class="col-md-4 control-label">File Prefix</label>

				<div class="col-md-6">
					<input id="file_prefix" type="text" class="form-control info-form-control"
						name="file_prefix"> <span id="file_prefix-span"
						class="help-block our-help-block"> <!-- IIIIIIIIIII --> <strong
						id="file_prefix-strong" class="our-error-message-strong"></strong> <!-- IIIIIIIIIII -->
					</span>
				</div>
			</div>
			...
			<div id="info-panel-buttons" class="form-group hide">
				<div class="col-md-6 col-md-offset-4">
					<button id="add-button" type="submit"
						...
						<i class="fa fa-btn fa-plus-circle"></i> Add Notice Type
					</button>
...
<div id="datatable-panel" class="panel panel-default">
	<div id="datatable-panel-heading" class="panel-heading clearfix">
		<div class="panel-title pull-left">Notice Type Master</div>
...
$(function() {
	table = $('#datatable-table').DataTable({
		...
		"serverSide": true,
		"ajax":{
			url:"{{url('admin/Notice-Type')}}-Datatable-Server-Side",
...
function populate_form(data){
	$("#info-panel-heading").html("Displaying record of Notice Type: <strong>"+data.type+"</strong>");

	$("#notice_type-id").val(data.id);
	
	$("#type").val(data.type);
	$("#description").val(data.description);
	$("#file_prefix").val(data.file_prefix);
}
...
function send_ajax_and_set_errors_exceptions_success(type){
	...
	if(type=="add"){
		...
		ajax_url="{{ action('Notice\NoticeTypeController@store') }}";
		...
		ajax_url="{{ action('Notice\NoticeTypeController@update','') }}"+"/"+$("#notice_type-id").val();
		formData["id"]=$("#notice_type-id").val();
		...
		ajax_url="{{ action('Notice\NoticeTypeController@destroy','') }}"+"/"+$("#notice_type-id").val();
		formData["id"]=$("#notice_type-id").val();
	...
	$.ajax({
		...
		success: function (data, textStatus, jqXHR) {
			...
			if(!(data.notice_type===null) && data.notice_type.hasOwnProperty('type')){
				//add and updattypeeDelete=> !(data.notice_type===null)
				msg+="Notice Type: <strong>"+data.notice_type.type+"</strong> successfully "+operated+".";
			}
			else{
				//delete case
				if(!(data.notice_type===null) && data.notice_type>=1){
					msg+="Notice Type: <strong>"+formData.type+"</strong> successfully "+operated+".";
				}
				else{
					msg+="Notice Type already "+operated+"!";
				}
			}
