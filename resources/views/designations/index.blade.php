{{--
<!-- views/designations/index.blade.php -->
--}}
@extends('layouts.app') @section('title', 'Designations')
@section('page_heading') Designations @endsection
...
function send_ajax_and_set_errors_exceptions_success(type){
	...
	if(type=="add"){
		...
		ajax_url="{{ action('DesignationController@store') }}";      
		operation="add";
		operated="added";
	}
	else if(type=="save"){
		...
		ajax_url="{{ action('DesignationController@update','') }}"+"/"+$("#designation_id").val();
		operation="update";
		operated="updated";
	}
		...
		formData["id"]=$("#designation_id").val();
		ajax_url="{{ action('DesignationController@destroy','') }}"+"/"+$("#designation_id").val();	
		operation="delete";
		operated="deleted";
	}
	$.ajax({
